...
def main():
    ...
    if signal_data:
        signal_data['letter'] = [label] * len(signal_data['0x'])
         # Convert to DataFrame
        new_signals = pd.DataFrame(signal_data)
    
        # Try reading existing CSV or create a new one if it doesn't exist
        try:
            existing_signals = pd.read_csv('hand_signals.csv')
            updated_stats = pd.concat([existing_signals, new_signals], ignore_index=True)
        except FileNotFoundError:
            updated_stats = new_signals  # If no existing file, just use new data

        # Save back to CSV
        updated_stats.to_csv('hand_signals.csv', index=False)
        print("New gesture data saved successfully!")
        print(label)
